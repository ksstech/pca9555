// pca9555.c - Copyright (c) 2014-26 Andre M. Maree / KSS Technologies (Pty) Ltd.
...
int pca9555Report(report_t * psR) {
	...
	if (sPCA9555.psI2C) {
		// ChkOK/ChkRst are register-consistency self-check results, NOT I2C transaction counts:
		// the I2C health metrics for this device are SEMerr/CFGerr/Txn on the halI2C line above.
		iRV += xReport(psR, "Inp=0x%04hX  Out=0x%04hX  Pol=0x%04hX  Cfg=0x%04hx  ChkOK=%lu  ChkRst=%lu" strNLx2,
